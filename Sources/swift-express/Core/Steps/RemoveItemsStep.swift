...
struct RemoveItemsStep : Step {
    ...
    func run(_ params: [String: Any], combinedOutput: StepResponse) throws -> [String: Any] {
        ...
        if let i = self.items {
            removeItems.append(contentsOf: i)
        }
        ...
        for item in removeItems {
            let path = workingFolder.appendingPathComponent(item)
            do {
                if FileManager.default.directoryExists(at: path) || FileManager.default.fileExists(at: path) {
